...
TILED_MAP = pathlib.Path(TILED_MAP)
...
def e_move(sprite):
    ...
    if sprite.left < MAP_LEFT:
        sprite.left = MAP_LEFT
    if sprite.bottom < MAP_DOWN:
        sprite.bottom = MAP_DOWN
    if sprite.center_x > MAP_RIGHT - 32:
        sprite.center_x = MAP_RIGHT - 32
    if sprite.center_y > MAP_UP - 32:
        sprite.center_y = MAP_UP - 32
...
def receive():
    while True:
        ...
        # enemies hurt :(
            ...
            if len(enemies_died) > index and enemies_died[index]: continue
                ...
                # pss pks type - players and enemies info
                    ...
                    for index, sprite in enumerate(enemies_sprites_list):
                        ...
                        if get_to_list_conditions:
                            ...
                            if enemies_died and enemies_died[index] and enemies_died_time\
                                    and time.time() - enemies_died_time[index] >= 0.3:  # after die - when revive - send info about the reviving pos
...
def s_main():
    slave.bind(ADDR)
    my_ip = sys.argv[1]
    send_response(slave, f'{my_ip}'.encode(), public_key, private_key, ('127.0.0.1', LB_PORT))
    # enemies start
