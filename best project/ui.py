...
class UI:
    ...
    def draw_magic_block(self):
        ...
        if self.player.magic is not None:
            ...
            arcade.draw_scaled_texture_rectangle(self.x + 64, self.y, magic_texture, 1.2)
    ...
    def draw_drop_block(self):
        ...
        if self.player.drop is not None:
            file_name = self.player.items[self.player.drop]['graphic']
            drop_texture = arcade.load_texture(file_name)
            arcade.draw_scaled_texture_rectangle(self.x + 128, self.y, drop_texture, 1.2)

            amount = self.player.items[self.player.drop]['amount']
            if amount != 'permanent':
